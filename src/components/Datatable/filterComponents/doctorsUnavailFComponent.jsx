...
import { doctorsUnavailFilterColumns } from "../data";
import { ChevronDown } from "lucide-react";
import moment from "moment";

export default function DoctorsUnavailFComponent({ table }) {
  const [selectedColumn, setSelectedColumn] = useState("doctor_details_name");
  ...
  const selectedDerivedColumn = useMemo(() => {
    return doctorsUnavailFilterColumns.find((v) => v.value === selectedColumn);
  }, [selectedColumn]);
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent className="w-56">
          ...
          <DropdownMenuRadioGroup value={selectedColumn} onValueChange={setSelectedColumn}>
            {doctorsUnavailFilterColumns.map((column) => {
              return (
          ...
          {selectedColumn && (
            <>
              ...
              <div className="flex justify-center">
                <Button variant="ghost" onClick={() => setSelectedColumn("doctor_details_name")}>
                  Reset Filter
                </Button>
              </div>
            </>

...
import { doctorsFilterColumns } from "../data";
import { ChevronDown } from "lucide-react";

export default function DoctorsFComponent({ table }) {
  const [selectedColumn, setSelectedColumn] = useState("name");
  
  const selectedDerivedColumn = useMemo(() => {
    return doctorsFilterColumns.find((v) => v.value === selectedColumn);
  }, [selectedColumn]);
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent className="w-56">
          ...
          <DropdownMenuRadioGroup value={selectedColumn} onValueChange={setSelectedColumn}>
            {doctorsFilterColumns.map((column) => {
